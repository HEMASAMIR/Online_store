...
// Final 100% Unique, verified active Unsplash images matching the products
...
const updated = products.map(p => {
  if (uniqueImageMap[p.id]) {
    return { ...p, image: uniqueImageMap[p.id] };
  }
  return p;
});

writeFileSync("./data/products.json", JSON.stringify(updated, null, 2), "utf-8");
console.log("data/products.json successfully updated with unique verified images.");
